...
function Matrix(){
	...
	this.rownum = 0;
    this.getcolumn = function(index){return this.matrix[index];};
    ...
    this.setcolumn = function(index, array){
        if(array.length != this.matrix[0].length){post("setcolumn receiving an array with wrong size\n");}
        else{this.matrix[index]=array;}
    }
    this.setrow = function(index, array){
        if(array.length != this.matrix.length){post("setrow receiving an array with wrong size\n");}
        else{
            for(i=0;i<this.matrix.length;i++){
...
function shortIndex(){
    ...
    this.add = function(index){
        ...
        this.inArray.sort(function(a, b){return a - b;});
...
function msg_int(cc, value){
	...
	if(changeCC){
		...
		if (ginterface){mgraphics.redraw();}
	...
	if(true){
	    ...
	    with(mgraphics){
	        ...
	        if (currentcolumn.length>0){
	            for (i=0; i < currentcolumn.length; i++) {
	                set_source_rgba(sliderColor[0], sliderColor[1], sliderColor[2], sliderColor[3]);
	                //sliders
	                rectangle_rounded(module_width*i+text_width, (1-currentcolumn[i])*dis_height, slider_width, dis_height, 0,0,0,0);
	                //text
	                startpoint = module_width*i+3;
	                fill();
	                set_source_rgba(0.,0.,0.,0.5);
	                identity_matrix;
	                move_to(startpoint,dis_height/10.);
	                rotate(1.570796);
	                show_text(namelist[i]);
	                identity_matrix();
	            }
	        }
	    }
...
function mapping(on){
    ...
    if(on){
        ...
        for (i=0; i < objlist.length; i++){
            ...
            //ubutton
            var ubtn = objlist[i].patcher.newdefault(x, y, "ubutton",
...
function ondrag(x,y,button){
    if(button){
        if(isover>-1){
            if(y<0){y=0;}
            if(y>dis_height){y=dis_height;}
            currentcolumn[isover]=1-y/dis_height;
            mgraphics.redraw();
        }
    }
    else{weightingsMatrix.setcolumn(shortCC.getShort(lastCC), currentcolumn);}
}
